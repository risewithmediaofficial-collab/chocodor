...
// 6. Raw Material / Inventory Model
...
// 8. Stock Movement Ledger Model
const stockMovementSchema = new mongoose.Schema({
  id: { type: String, required: true, unique: true, index: true },
  material_id: { type: String, required: true, index: true },
  type: { type: String, required: true }, // 'IN' | 'OUT' | 'ADJUSTMENT' | 'ORDER_CONSUMPTION'
...
const productSchema = new mongoose.Schema({
  ...
  takeaway_extra_cost: { type: Number, default: 0 },
...
const orderSchema = new mongoose.Schema({
  ...
  takeaway_extra_total: { type: Number, default: 0 },
  ...
  stock_deducted: { type: Number, default: 0 },
  stock_reversed: { type: Number, default: 0 },
  ...
  payment_method: { type: String, default: 'COD' }, // 'COD' | 'RAZORPAY' | 'CASH' | 'UPI' | 'CARD' | 'SPLIT'
  payment_breakdown: { type: Array, default: [] },
...
// 12. Kitchen Order Ticket (KOT) Model
...
// 16. Reward Redemptions (Coupon Codes) Model
...
// 20. QR Tokens Model (In-Store QR Fast Login)
...
export const CategoryMaterial = mongoose.models.CategoryMaterial || mongoose.model('CategoryMaterial', categoryMaterialSchema)
